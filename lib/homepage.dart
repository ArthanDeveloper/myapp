  ...
  Future<void> loadDashBoard() async {
       ...
       try {
        final prefs = await SharedPreferences.getInstance();
         final String? customerId = prefs.getString('customerId'); //what was the string
               if (customerId == null) {
                    ScaffoldMessenger.of(context).showSnackBar(
                       const SnackBar(content: Text('This is internal error please try to login again contact arthik Support and provide what has happened in screen')),
                     );
        return;
       }
         final response = await _apiService.fetchAccountsByCustomerId(customerId);
          ...
          final List<dynamic> loanData = response['data'];
              _loans = loanData.map((item) => Loan(
               ...
               lastUpdated: item['accountOpenDateStr'] ?? 'Date not available',
                accountId: item['accountId'] ?? 'NA',
              )).toList();
           

           });
        debugPrint('API Response: $response'); // Print the entire response to check all
    } else {
     ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Data load issue! contact Support if issue persists')),
      );
     return;
     }
      } catch (e) {
              ...
              ..._loans.map((loan) => Card(
                ...
                child: InkWell(
                  onTap: () {
                    Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => LoanDetailsScreen(loan: loan)),
                     );
                  },
                  borderRadius: BorderRadius.circular(16.0),
                  child: Padding(
